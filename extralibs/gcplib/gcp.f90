...
!*              g C P                                          *
!***************************************************************
! Copyright (C) 2012 Stefan Grimme, University of Bonn, Germany
! 
...
!
! gcp_call(n,xyz,lat,iz,gcp_e,gcp_g,gcp_glat,dograd,dohess,pbc,method,echo,parfile)
!
! program gcp_main
! use strings
! implicit none
...
subroutine gcp_call(n,xyz,iz,gcp_e,gcp_g,dograd,dohess,method,echo,parfile,fileio)
implicit none
integer n   !number of atoms
integer fileio ! connect to output unit
...
character(*) method !method string
...
character*80 getlevel
!v2.0 stuff
logical tmen,base,damp,stress,parfile
character*80 args(90), comment, line
...
if(echo)then 
write(fileio,*)' ___________________________________________ '
write(fileio,*)'                                             '
write(fileio,*)'|              **  g C P  **                |'
write(fileio,*)'|  a geometrical counterpoise correction    |'
write(fileio,*)'|     H.Kruse J.G.Brandenburg S.Grimme      |'
write(fileio,*)'|          Version 1.06 Sep 2014            |'
write(fileio,*)'|          HK (library version              |'
write(fileio,*)' ___________________________________________ '
write(fileio,*)
write(fileio,*)'Please cite work done with this code as:'
write(fileio,*)'H. Kruse, S. Grimme J. Chem. Phys. 136, 154101 (2012)'
write(fileio,*)'DOI: 10.1063/1.3700154 '
write(fileio,*)'For the periodic version, please also cite:'
write(fileio,*)'J. G. Brandenburg, M. Alessio, B. Civalleri, M. F. Peintinger'
write(fileio,*)'T. Bredow, S.Grimme J. Phys. Chem. A 117, 9282-9292 (2013).'
write(fileio,*)'DOI: 10.1021/jp406658y '
write(fileio,*)
write(fileio,*)
endif
...
if(echo) write(fileio,*) ' Perform special gCP-SRB correction for HF-3c '
base=.true.
case('pbeh3c')
!JGB special method combination 2: PBEh-3c
if(echo) write(fileio,*) ' Perform special gCP(damped) correction for PBEh-3c'
damp=.true.
case default
end select
...
!  close(43,status='delete')  
  ...
  if(ex)then
     if(echo)write(fileio,*) 'reading ',trim(dtmp)
     ...
     p(1:4)=xx(1:4)
      if(adjustl(trim(basname)).eq.'#') then
       if(echo) then
       write(fileio,*) ' found extended param file ! '
       endif
       parm=.true.    
...
!additional read for new param file
      ...
      if(echo) then
       write(fileio,*) ' found extended param file v2! '
      endif
      call readl(ftmp,xx,nn)
      parm=.true. 
      damp=.true.
...
9   close(42)
     if(.not.parm) then
      if(echo) write(fileio,*) 'loading ',trim(basname),' params'
      call setparam(emiss,nbas,p,basname)
     endif
  else
   write(fileio,'(a)') ''
   write(fileio,'(2a)')  'found not param file in ',trim(dtmp)
   stop '* ERROR *'
  endif
method=trim(basname)
!*******************
...
select case (zz)
 case(37:54)
  iz(i)=iz(i)-18
  write(fileio,'(4a)') '** NOTE ** -> element ',esym(zz),' will be treated as ', esym(iz(i))
  warn=.true.
 case(55:57)
  iz(i)=iz(i)-18*2
  write(fileio,'(4a)') '** NOTE ** -> element ',esym(zz),' will be treated as ', esym(iz(i))
...
write(fileio,'(a)') 'VMB basis has ECPs. Adjusting electrons' 
do i=5,10
nel(i)=nel(i)-2
enddo
do i=11,18
...
do i=1,n
  ...
  write(fileio,'(a)') 'element/emiss/nvirt/nel/nbas'
  write(fileio,'(a4,f8.5,f8.5,i3,i3)') esym(iz(i)),emiss(iz(i)),xva(i),nel(iz(i)),nbas(iz(i))
...
!    xva(i)=0
  ...
  write(fileio,'(3a)') '** WARNING ** -> element ',esym(iz(i)),' has no virtual orbitals (no contribution)!' 
  warn=.true.
  endif
enddo
xvb=xva
...
!*********************************
  if(echo)then
     write(fileio,*)''
     write(fileio,'(2x,''level '',3x,a12,2x,a)') adjustr(trim(method)),adjustr(trim(getlevel(method)))
     write(fileio,'(2x,''Nbf   '',3x,I12)')  nb
     write(fileio,'(2x,''Atoms '',3x,I12)')  n
     write(fileio,*)''
     write(fileio,'(2x,a)') 'Parameters: '
     write(fileio,'(2x,a,2x,f10.4)') 'sigma   ',p(1)
     write(fileio,'(2x,a,2x,f10.4)') 'eta     ',p(2)
     write(fileio,'(2x,a,2x,f10.4)') 'alpha   ',p(3)
     write(fileio,'(2x,a,2x,f10.4)') 'beta    ',p(4)
   if(damp) then
     write(fileio,'(2x,a,2x,f10.4)') 'dmp_scal',dmp_scal
     write(fileio,'(2x,a,2x,f10.4)') 'dmp_exp ',dmp_exp
   endif
     write(fileio,*)''
   endif
...
if(echo) then
  write(fileio,*) ' '
  write(fileio,*) 'element parameters ',trim(method)
  write(fileio,'(2x,3(a4,2x,a6,3x,a4,4x))') 'elem','emiss','nbas','elem','emiss','nbas','elem','emiss','nbas'
  do i=1,36,3
      write(fileio,'(2x,3(A4,2x,F8.5,2x,I3,4x))') esym(i), emiss(i),nbas(i), &
...
! **************************
if(parfile) then
  write(fileio,'(a)')  'printing extended parameter file ...'
  write(fileio,'(a)')  'modfiy & copy gcp.param to $HOME/.gcppar.$HOSTNAME'
...
gcp_g(1:3,1:n)=0.0d0
gcp_e=0.0d0
call gcp_egrad(n,max_elem,emiss,xyz,iz,p,gcp_e,gcp_g,dograd,echo,xva,xvb,pbc,lat,damp,base,dmp_scal,dmp_exp,fileio)
...
if(echo) write(fileio,'(a)') ' setting gradient of frozen cartesians to zero'
  do i=1,n
...
if(echo) then
 write(fileio,*) ' '
 write(fileio,*) '|S|=',abs(sum(gcp_glat(1:3,1:3)))
 if(.not.verb) then
 write(fileio,'(a)')  'Write cell gradient to file gcp_cellgradient'
...
if(echo)then
  write(fileio,*) '                '
  write(fileio,*) '** gCP correction ** '
  write(fileio,'(2x,a7,F18.10,'' / (a.u.) || '',x,F11.4,'' / (kcal/mol)'')') 'Egcp:  ', gcp_e,gcp_e*627.5099d0
  write(fileio,*) '                '
    if(dograd)then
    write(fileio,*)'|G|=',sum(abs(gcp_g(1:3,1:n)))
    endif
endif
...
!***************************************************
subroutine gcp_egrad(n,max_elem,emiss,xyz,iz,p,gcp,g,grad,echo,xva,xvb,pbc,lat,damp,base,dmp_scal,dmp_exp,fileio)
implicit none             
integer n,iz(n),max_elem,np
integer iat,jat,fileio
...
if(echo) then
write(fileio,*) '  '     
write(fileio,'('' cutoffs: '',F5.1)') 
write(fileio,'(''   distance [bohr]'',F5.1)') thrR
write(fileio,'(''   noise    [a.u.]'',Es8.1)') thrE
write(fileio,'(''   SR-damping     '',L2)') damp
write(fileio,*) '  '     
endif
...
call setzet(p2,za,zb)
   gs=0
   if(echo) write(fileio,'(2x,a5,2x,a5,2x,a5,2x,a7,2x,a14,4x,a15)') &
...
!         write(fileio,*) 'a1 a2 a3',  lat(1,1),lat(1,2),lat(1,3)
!         write(fileio,*) 'b1 b2 b3',  lat(2,1),lat(2,2),lat(2,3)
!         write(fileio,*) 'c1 c2 c3',  lat(3,1),lat(3,2),lat(3,3)
!         stop
...
!end supercell loop
   ...
   enddo
   if(echo) &
    write(fileio,'(2x,3(I5,2x),2x,F5.1,2x,F14.4,2x,F14.4,2x,a)')  &
...
data HFdef1tzvp /&
0.007577,0.003312,&
0.136371,0.011163,0.017129,0.008140,0.005826,0.006770,0.007108,0.008132,&
...
! ! endif
! write(*,*) 'G(gcp) written to file gcp_gradient'
! open(unit=142,file='gcp_gradient')
! do i=1,nat
...
!             xyz,iz,p,g/cp,gr,.true.,.false.,xva,xvb,pbc,lat,damp,base,dmp_scal,dmp_exp)
